...
def index():
    ...
    if current_user.role in ('manager', 'admin'):
        approval_pending_count += WorkRequest.query.filter_by(status='pending_manager').count()
        approval_pending_count += ManpowerRequest.query.filter_by(status='pending_manager').count()
        approval_pending_count += SupplyRequest.query.filter_by(status='pending_manager').count()
    if current_user.role in ('cluster_manager', 'admin'):
        approval_pending_count += WorkRequest.query.filter_by(status='pending_cm').count()
        approval_pending_count += ManpowerRequest.query.filter_by(status='pending_cm').count()
        approval_pending_count += SupplyRequest.query.filter_by(status='pending_cm').count()
